...
-- Create or update admin user profile for paul@spryfi.net
INSERT INTO public.user_profiles (user_id, email, role, first_name, last_name, initials, is_active)
SELECT 
  auth.users.id,
  'paul@spryfi.net',
  'admin',
  ...
  true
FROM auth.users 
WHERE auth.users.email = 'paul@spryfi.net'
ON CONFLICT (user_id) 
DO UPDATE SET 
  role = 'admin',
  email = 'paul@spryfi.net',
  is_active = true;